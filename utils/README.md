...
(FYI: The data on https://emojiterra.com/ is also sourced from the unicode repository)

If the language exists, open the `{language-code}.xml` file and check if there are actually translations for emoji.
We use the entries that have the attribute `type="tts"` because they only contain one translation, so we don't have to decide which to use.
...
This would result in the emoji name `:gato_haciendo_una_mueca:`

## Generate EMOJI_DATA

To generate all these names automatically, we use [`utils/get_codes_from_unicode_emoji_data_files.py`](utils/get_codes_from_unicode_emoji_data_files.py)
